...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'POST') {
    const { targetEmail = 'Jonathan.rendon@gmail.com' } = req.body || {};
    ...
    if (!config.isConfigured) {
      return res.status(200).json({
        ...
        troubleshooting: [
          'Opción A (Recomendada - Gmail): Agrega en Vercel las variables SMTP_USER (tu gmail) y SMTP_PASS (tu contraseña de aplicación de 16 letras de Google). No requiere comprar dominio.',
          'Opción B (Resend): Agrega RESEND_API_KEY y configura tu dominio en resend.com/domains. Nota: con onboarding@resend.dev sólo puedes enviar a tu propio correo registrado.'
        ],
        config
      });
    }
    ...
    if (result.sent) {
      ...
      return res.status(200).json({
        success: false,
        message: `Falló el envío de correo con ${result.provider || config.provider}.`,
        error: result.error || result.reason,
        troubleshooting: result.provider === 'resend' && String(result.error).includes('only send testing emails')
          ? 'Resend con dominio de prueba (onboarding@resend.dev) sólo permite enviar al correo con el que te registraste en Resend. Para enviar a otros (ej. Jonathan y Michelle), debes verificar un dominio propio en resend.com o usar SMTP de Gmail (SMTP_USER y SMTP_PASS).'
          : 'Verifica las credenciales en Vercel Environment Variables y vuelve a desplegar.',
        result
      });
    }
  }
